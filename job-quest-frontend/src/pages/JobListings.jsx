...
const JobListings = () => {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      setIsLoading(true);
      const jobsResponse = await api.get("/jobs"); // ✅ cleaned
  ...
  const applyForJob = async (formData) => {
    try {
      const applyResponse = await api.post("/applications", formData); // ✅ cleaned
  ...
  const deleteJob = async (job) => {
    ...
    try {
      const deleteResponse = await api.delete(`/jobs/${job.id}`); // ✅ cleaned

      if (deleteResponse.status === 204) {
        const removeResponse = await api.post(
          `/recruiters/${userData.email}/removejob`, // ✅ cleaned
          job.id
        );
