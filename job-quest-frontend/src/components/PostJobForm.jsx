...
const PostJobForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const jobResponse = await api.post("/jobs", formData); // ✅ cleaned

      if (jobResponse.status === 201) {
        const appendResponse = await api.post(
          `/recruiters/${userData?.email}/appendjob`, // ✅ cleaned
          jobResponse.data.id
        );
  ...
  return (
    ...
    >
      ...
      <input
        ...
        className="w-full py-2 px-4 text-lg rounded-lg text-black/80 font-semibold mt-6"
        required
      />
