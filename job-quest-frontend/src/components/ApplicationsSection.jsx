...
const ApplicationsSection = () => {
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      ...
      console.log("All applications:", applicationsData);
          console.log(" jobIds in each application →", applicationsData.map(a => a.jobId));
          console.log("Recruiter jobIds:", userData?.jobIds);
    ...
    const recruiterApplications = applicationsData.filter((application) => {
      return (
        Array.isArray(userData?.jobIds) &&
        userData.jobIds.includes(String(application.jobId))
      );
    });

        const formattedApplications = recruiterApplications.map((application) => {
          const jobInfo = jobsData.find((job) => job.id === application.jobId);
          return {
            ...application,
        ...
        setPendingApplications(formattedApplications.filter((item) => item.status === "Pending"));
        setAcceptedApplications(formattedApplications.filter((item) => item.status === "Accepted"));
        setRejectedApplications(formattedApplications.filter((item) => item.status === "Rejected"));
      } else {
  ...
  const acceptApplication = async (item) => {
    ...
    try {
      const response = await api.post(`/applications/${item.id}`, "Accepted");

      if (response.status === 200) {
        setPendingApplications(
          pendingApplications.filter((application) => application.id !== item.id)
        );
  ...
  const rejectApplication = async (item) => {
    ...
    try {
      const response = await api.post(`/applications/${item.id}`, "Rejected");
  ...
  const renderCandidateApplicationDetailsLeftSide = (item) => (
    <div className="p-6 w-4/5">
      ...
      <div>
        ...
        <a href={item.resumeLink} target="_blank" rel="noreferrer" className="underline">
          Resume
        </a>
      </div>
    </div>
      ...
      <div className="my-8 flex flex-col gap-6 text-white">
        {isLoading ? (
          ...
          pendingApplications.map((item) => (
            <div key={item.id} className="flex justify-between divide-x-2 border rounded-lg">
      ...
      <div className="my-8 flex flex-col gap-6 text-white">
        {isLoading ? (
          ...
          acceptedApplications.map((item) => (
            ...
            >
              {renderCandidateApplicationDetailsLeftSide(item)}
              <div className="px-6 w-1/5 flex flex-col justify-center items-center text-white">
